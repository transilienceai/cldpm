"""Implementation of cpm get command."""
...
from ..core.config import load_cpm_config
...
def get(
    ...
    \b
    Examples:
      cpm get my-project                      # Show project info
      cpm get my-project -f json              # JSON output
      cpm get my-project -d                   # Download to ./my-project
      cpm get my-project -d -o ./copy         # Download to ./copy
      cpm get my-project -r owner/repo        # From remote
      cpm get my-project -r owner/repo -d     # Download remote
    """
    if remote_url:
        _handle_remote_get(
...
def _handle_remote_get(
    ...
    try:
        ...
        # Check if it's a valid CPM repo
        if not (temp_dir / "cpm.json").exists():
            print_error("Remote repository is not a CPM mono repo (no cpm.json found)")
            raise SystemExit(1)
...
def _download_remote_project(
    ...
    cpm_config = load_cpm_config(temp_dir)
    shared_dir = temp_dir / cpm_config.shared_dir
    source_path = Path(resolved["path"])
