"""Implementation of cpm link command."""
...
from ..core.config import load_cpm_config, load_component_metadata
...
def link(dependencies: str, target: str) -> None:
    ...
    Examples:
      ...
      cpm link rule:security --to agent:security-audit
    ...
    # Load CPM config to check if dependencies exist
    cpm_config = load_cpm_config(repo_root)
    shared_dir = repo_root / cpm_config.shared_dir
...
def unlink(dependencies: str, target: str) -> None:
    ...
    Examples:
      ...
      cpm unlink rule:security --from agent:security-audit
