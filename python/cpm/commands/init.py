"""Implementation of cpm init command."""
...
from ..schemas import CpmConfig, ProjectConfig, ProjectDependencies
from ..core.config import save_cpm_config, save_project_config
...
def init(
    ...
    Structure created:
      ...
      cpm.json         - Mono repo configuration
      CLAUDE.md        - Root instructions

    \b
    Examples:
      cpm init                              # Current directory
      cpm init my-monorepo                  # New directory
      cpm init --name "My Repo"             # Custom name
      cpm init -e                           # Existing repo, don't overwrite
      cpm init -e --adopt-projects auto     # Auto-detect existing projects
      cpm init -e -a "app,api"              # Adopt specific projects
      cpm init -p src -s common             # Custom directory names
    """
    # Resolve directory
    repo_root = Path(directory).resolve()

    # Check if already initialized
    if (repo_root / "cpm.json").exists():
        if existing:
            print_warning(f"CPM repo already exists at {repo_root}, updating...")
        else:
            print_error(f"CPM repo already exists at {repo_root}")
            raise SystemExit(1)
    ...
    if not existing or not gitignore_path.exists():
        ...
        # Append CPM-specific entries if not already present
        _update_gitignore(gitignore_path, projects_dir)

    print_success(f"Initialized CPM mono repo: {name}")
...
def _update_gitignore(gitignore_path: Path, projects_dir: str) -> None:
    """Update existing .gitignore with CPM-specific note."""
    existing_content = gitignore_path.read_text()

    # Check if CPM note already exists
    if "CPM Note" in existing_content or "CPM shared components" in existing_content:
        return

    cpm_note = """
# CPM Note: Shared component symlinks are managed per-directory
...
def _adopt_projects(
    ...
    """Adopt existing directories as CPM projects.
        ...
        if not candidates:
            for item in repo_root.iterdir():
                if (
                    ...
                    and item.name not in ["shared", "projects", ".cpm", "node_modules", "__pycache__", "venv", ".venv"]
                ):
...
def _setup_project_structure(
    ...
    """Set up the standard CPM project structure."""
