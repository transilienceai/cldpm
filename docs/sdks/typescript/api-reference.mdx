...
Root configuration for `cpm.json`.
...
```typescript
function loadCpmConfig(repoRoot: string): Promise<CpmConfig>
```

**Throws:** Error if `cpm.json` not found.
...
```typescript
function saveCpmConfig(config: CpmConfig, repoRoot: string): Promise<void>
```
