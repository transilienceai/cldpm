...
```bash
cpm add skill:documentation --to my-project
cpm add agent:debugger --to my-project
cpm add rule:security --to my-project
```
...
for project in web-app api-server worker; do
  cpm add skill:testing --to $project
  cpm add rule:security --to $project
done
```
...
```bash
cpm remove skill:testing --from my-project
```

### Verify components

```bash
cpm get my-project --format json | jq '.shared'
```
