...
```bash
cpm remove <COMPONENT> --from <PROJECT>
```
...
```bash
cpm remove skill:code-review --from my-project
```

### Remove with short flag

```bash
cpm remove agent:debugger -f my-project
```

### Keep orphaned dependencies

```bash
cpm remove skill:main-skill --from my-project --keep-deps
```

When a component has dependencies that no other component uses, CPM will prompt to remove them. Use `--keep-deps` to skip this.

### Force removal

```bash
cpm remove skill:shared-util --from my-project --force
```

If other components in the project depend on the component being removed, CPM will error by default. Use `--force` to remove anyway.

## Dependency Handling

### Orphaned Dependencies

When you remove a component, CPM checks if any of its dependencies are still used:
...
```bash
$ cpm remove skill:base-util --from my-project
